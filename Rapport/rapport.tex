...
\medskip
La démarche a consisté à concevoir et développer une application complète (backend, base de données, frontend et conformité réglementaire), en suivant une approche itérative. Le système repose sur une architecture \textbf{3-tiers} (FastAPI, React/Vite, PostgreSQL) avec \textbf{14 tables} de base de données, \textbf{83 endpoints} API, \textbf{14 routes} frontend et \textbf{8 indicateurs de base} d'insertion professionnelle, complétés d'un indicateur dérivé pour chaque question taguée KPI active.
...
\begin{itemize}
  ...
  \item \textit{Espace alumni}~: inscription multi-étapes, vérification OTP, édition de profil, parcours professionnel (expériences et certifications gérées par ajout/suppression, la modification directe d'une expérience existante n'étant pas disponible à ce jour, limite assumée du prototype), consentement RGPD, questionnaire annuel.
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{8 indicateurs de base} d'insertion professionnelle, exposés via des endpoints dédiés (\texttt{/admin/indicateurs}, \texttt{/admin/indicateurs/secteurs}, \texttt{/admin/indicateurs/types-contrat}), complétés d'un endpoint agrégé partenaire et d'un indicateur dérivé par question taguée active (variable).
...
\textbf{Cas limites.} Promotion sans fenêtre de 6 mois écoulée $\rightarrow$ \texttt{null} + statut \og{}en\_attente\fg{}~; échantillon de salaire < 5 $\rightarrow$ fourchette élargie et mention \og{}échantillon limité\fg{}~; tag KPI sans réponse $\rightarrow$ état vide.
