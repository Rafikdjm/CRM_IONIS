...
\medskip
La démarche a consisté à concevoir et développer une application complète --- backend, base de données, frontend et conformité réglementaire --- en suivant une approche itérative. Le système repose sur une architecture \textbf{3-tiers} (FastAPI, React/Vite, PostgreSQL) avec \textbf{14 tables} de base de données, \textbf{83 endpoints} API, \textbf{14 routes} frontend et \textbf{8 indicateurs} d'insertion professionnelle.
...
J'ai développé une API REST complète avec FastAPI (Python). L'API comprend \textbf{16 routeurs} et \textbf{83 endpoints} couvrant~:
...
\begin{itemize}
  ...
  \item \textbf{83 endpoints} API avec authentification OTP + JWT et protection admin, dont les ajouts de fin de stage~: \texttt{POST /newsletter/envoyer} (newsletter avec filtres de ciblage), \texttt{POST /admin/questionnaires/notififier} (relance questionnaire avec filtres de ciblage et exclusion RGPD), \texttt{GET /admin/indicateurs/partenaires} (indicateurs agrégés anonymisés) et \texttt{GET /admin/demandes-rgpd/bulk/export} (export groupé compatible avec les téléchargements natifs mobiles).
...
L'API expose \textbf{83 endpoints} REST répartis sur \textbf{16 routeurs} (60 chemins). La liste exhaustive des méthodes et chemins est consultable directement dans la documentation Swagger générée automatiquement par FastAPI (endpoint \texttt{/docs}). Les principaux domaines sont~: Authentification (3), Promotions (5), Étudiants (7), Entreprises (5), Expériences (5), Certifications (8), RGPD alumni (7), RGPD admin (11), Annuaire/Indicateurs (8), Nettoyage (6), Import/Export (3), Questionnaires alumni (4), Questionnaires admin (9), Newsletter (1).
