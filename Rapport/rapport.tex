...
\begin{tikzpicture}[remember picture,overlay]
  % Bandeau supérieur \hyperlink{abbr-IONIS}{IONIS (18)}
...
\medskip
La démarche a consisté à concevoir et développer une application complète (backend, base de données, frontend et conformité réglementaire), en suivant une approche itérative. Le système repose sur une architecture \textbf{3-tiers} (FastAPI, \hyperlink{abbr-React}{React (37)}/\hyperlink{abbr-Vite}{Vite (47)}, \hyperlink{abbr-PostgreSQL}{PostgreSQL (33)}) avec \textbf{14 tables} de base de données, \textbf{83 endpoints} \hyperlink{abbr-API}{API (1)}, \textbf{14 routes} frontend et \textbf{8 indicateurs de base} d'insertion professionnelle, complétés d'un indicateur dérivé pour chaque question taguée \hyperlink{abbr-KPI}{KPI (23)} active.
...
\medskip
The approach followed an iterative cycle: data modeling, backend development, frontend implementation, security audit, and documentation. The resulting system is built on a 3-tier architecture (FastAPI, \hyperlink{abbr-React}{React (37)}/\hyperlink{abbr-Vite}{Vite (47)}, \hyperlink{abbr-PostgreSQL}{PostgreSQL (33)}) and includes \textbf{14 database tables}, \textbf{83 \hyperlink{abbr-API}{API (1)} endpoints}, \textbf{14 frontend routes}, and \textbf{8 professional insertion indicators}.
...
\begin{longtable}{@{} l l >{\RaggedRight\arraybackslash}p{12.6cm} @{}}
\toprule
...
Le passage du \hyperlink{abbr-MCD}{MCD (24)} au \hyperlink{abbr-MLD}{MLD (25)} a respecté les règles de transformation standard (entité forte $\rightarrow$ table, association \hyperlink{abbr-NM}{N:M (27)} $\rightarrow$ table de jonction). J'ai versionné 16 migrations \hyperlink{abbr-SQL}{SQL (42)}, appliquées via un script maison (\texttt{run\_migrations.py}) qui ne rejoue que les migrations non encore exécutées.

\subsection{Mission 2 : Développement du backend API}

J'ai développé une \hyperlink{abbr-API}{API (1)} \hyperlink{abbr-REST}{REST (39)} complète avec FastAPI (Python). L'\hyperlink{abbr-API}{API (1)} comprend \textbf{16 routeurs} et \textbf{83 endpoints} couvrant~:
...
\begin{itemize}
  ...
  \item \textit{Espace alumni}~: inscription multi-étapes, vérification \hyperlink{abbr-OTP}{OTP (28)}, édition de profil, parcours professionnel (expériences et certifications gérées par ajout/suppression, la modification directe d'une expérience existante n'étant pas disponible à ce jour, limite assumée du prototype), consentement \hyperlink{abbr-RGPD}{RGPD (40)}, questionnaire annuel.
\end{itemize}
...
\begin{itemize}
  ...
  \item Information de l'alumni dans l'interface de consentement~: durée de conservation des données (suppression 6 mois après anonymisation) et contact \hyperlink{abbr-DPO}{DPO (12)} (\texttt{contact@ionis-stm.com}).
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{83 endpoints} \hyperlink{abbr-API}{API (1)} avec authentification \hyperlink{abbr-OTP}{OTP (28)} + \hyperlink{abbr-JWT}{JWT (22)} et protection admin, dont les ajouts de fin de stage~: \texttt{\hyperlink{abbr-POST}{POST (32)} /newsletter/envoyer} (newsletter avec filtres de ciblage), \texttt{\hyperlink{abbr-POST}{POST (32)} /admin/questionnaires/notififier} (relance questionnaire avec filtres de ciblage et exclusion \hyperlink{abbr-RGPD}{RGPD (40)}), \texttt{\hyperlink{abbr-GET}{GET (15)} /admin/indicateurs/partenaires} (indicateurs agrégés anonymisés) et \texttt{\hyperlink{abbr-GET}{GET (15)} /admin/demandes-rgpd/bulk/export} (export groupé compatible avec les téléchargements natifs mobiles).
...
\begin{itemize}
  ...
  \item \textit{Rigueur méthodologique}~: audit de cohérence entre le modèle et l'implémentation réelle mené par introspection \hyperlink{abbr-SQL}{SQL (42)} (\texttt{information\_schema}, \texttt{pg\_constraint}), écrit et daté avant tout correctif (\texttt{AUDIT\_COHERENCE\_TABLES.txt}).
\end{itemize}
...
\textbf{Standardisation des contraintes de validation.} Le statut des consentements dans la table \texttt{CONSENTEMENT\_RGPD} reste libre (ni \texttt{Literal} côté \hyperlink{abbr-API}{API (1)}, ni \texttt{\hyperlink{abbr-CHECK}{CHECK (3)}} côté base). La date d'obtention des certifications n'est pas validée (une date future passe sans erreur). La mise en place de contraintes de validation à la source (types énumérés côté \hyperlink{abbr-API}{API (1)}, contraintes \hyperlink{abbr-CHECK}{CHECK (3)} côté base) est une bonne pratique à généraliser.
...
Infrastructure & otp\_codes, schema\_migrations & otp\_codes~: codes \hyperlink{abbr-OTP}{OTP (28)} hachés identifiés par l'email~; schema\_migrations~: suivi des 16 migrations versionnées \\
\bottomrule
\end{tabularx}
\end{table}
...
\textbf{Endpoints \hyperlink{abbr-API}{API (1)}.} \texttt{\hyperlink{abbr-GET}{GET (15)} /admin/indicateurs}, \texttt{/admin/indicateurs/secteurs}, \texttt{/admin/indicateurs/types-contrat}, \texttt{/admin/indicateurs/kpi-tag}, \texttt{/admin/indicateurs/kpi-tags}, \texttt{/admin/indicateurs/kpi-tags-actifs}, \texttt{/admin/indicateurs/partenaires}.

\textbf{Cas limites.} Promotion sans fenêtre de 6 mois écoulée $\rightarrow$ \texttt{null} + statut \og{}en\_attente\fg{}~; échantillon de salaire < 5 $\rightarrow$ fourchette élargie et mention \og{}échantillon limité\fg{}~; tag \hyperlink{abbr-KPI}{KPI (23)} sans réponse $\rightarrow$ état vide.
...
% ── 2. Demande d'un code \hyperlink{abbr-OTP}{OTP (28)} ─────────────────────────────────────
...
% ── 3. Vérification du code \hyperlink{abbr-OTP}{OTP (28)} ──────────────────────────────────
...
% ── 6. Tags \hyperlink{abbr-KPI}{KPI (23)} des questionnaires actifs ────────────────────────
...
\textbf{Points secondaires ouverts (assumés).} Statut des consentements libre (ni \texttt{Literal} ni \hyperlink{abbr-CHECK}{CHECK (3)}), date d'obtention des certifications non validée (une date future passe), réponses de questionnaire stockées en \hyperlink{abbr-JSONB}{JSONB (21)} sans vérification des clés, absence de purge des tables \texttt{otp\_codes} et \texttt{AUDIT\_LOG}. Le détail est consigné dans \texttt{AUDIT\_COHERENCE\_TABLES.txt}.
